...
//为服务器对象添加请求事件
app.on('request', async(req, res) => { //req请求 res响应

    const method = req.method; //判断传递方法 是post 还是get方法

    const { pathname, query } = url.parse(req.url, true); //通过数组解构 的方法的到相应内容
...
<body>
	<div class="container">
        ...
        if (pathname == '/add') {
            ...
            //post参数接受完毕
            req.on('end', async() => { //使用await需要添加async关键字 转化为promise对象
